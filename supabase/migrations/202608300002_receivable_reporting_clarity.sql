...
begin
  ...
  select coalesce(sum(s.total_minor),0)::bigint,count(*)::integer,
    coalesce(round(avg(s.total_minor)),0)::bigint,coalesce(sum(s.tax_minor),0)::bigint
  into gross_sales,transaction_total,average_ticket,tax_total
  from public.sales s where s.tenant_id=tenant and s.branch_id=target_branch_id and s.status='paid'
  ...
  with paid as (
    ...
    select case when s.payment_method='credit' then 'cash' else s.payment_method::text end method,
      case when s.payment_method='credit' then s.paid_minor else s.total_minor end::bigint amount_minor
    from current_sales s
    union all
    select 'receivable',greatest(s.total_minor-s.paid_minor,0)::bigint from current_sales s
    where s.payment_method='credit' and s.total_minor>s.paid_minor
  ), payment_mix as (
