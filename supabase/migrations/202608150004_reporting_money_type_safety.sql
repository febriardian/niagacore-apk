...
begin
  ...
  with document_types(document_type) as (values ('receivable'::text),('payable'::text)), documents as (
    ...
    'inputTaxMinor',coalesce((select sum(coalesce(nullif(metadata->>'taxMinor','')::bigint,0)) from public.business_records where tenant_id=tenant and branch_id=target_branch_id and kind in('supplier_bill','expense') and status='posted'),0)::bigint,
