...
-- Dashboard kasir hanya membaca transaksi milik akun yang sedang masuk.
-- Kas diterima memakai waktu pembayaran sehingga cicilan lanjutan masuk ke
-- periode saat uang benar-benar diterima, bukan kembali ke tanggal penjualan.
...
begin
  ...
  from public.sales s
  where s.tenant_id=tenant and s.branch_id=target_branch_id and s.cashier_id=actor and s.status='paid'
  ...
  from public.sales s
  where s.tenant_id=tenant and s.branch_id=target_branch_id and s.cashier_id=actor
...
-- Paksa PostgREST membaca fungsi baru segera setelah migrasi selesai.
